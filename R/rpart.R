...
rpart <-
					 ...
					 ...)
	{
		...
		if (is.data.frame(model)) {
			...
			temp <-
				Call[c(1L, indx)]      # only keep the arguments we wanted
			temp$na.action <- na.action    # This one has a default
			temp[[1L]] <-
				quote(stats::model.frame) # change the function called
			m <- eval.parent(temp)
		}
		...
		xval <- controls$xval
		if (is.null(xval) ||
				(length(xval) == 1L && xval == 0L) || method == "user") {
			xgroups <- 0L
			xval <- 0L
		} else if (length(xval) == 1L) {
			## make random groups
			xgroups <-
				sample(rep(1L:xval, length = nobs), nobs, replace = FALSE)
		...
		tfun <- function(x)
			...
			is.ordered(x)
		labs <-
			sub("^`(.*)`$", "\\1", attr(Terms, "term.labels")) # beware backticks
		...
		if (!length(temp))
			temp <- 0    # if parms is NULL pass a dummy
		rpfit <- .Call(
			C_rpart,
		...
		nsplit <-
			nrow(rpfit$isplit) # total number of splits, primary and surrogate
		...
		if (nsplit == 0L)
			xval <- 0L # No xvals were done if no splits were found

		numcp <- ncol(rpfit$cptable)
		temp <-
			if (nrow(rpfit$cptable) == 3L)
				c("CP", "nsplit", "rel error")
		else
			c("CP", "nsplit", "rel error", "xerror", "xstd")
		dimnames(rpfit$cptable) <- list(temp, 1L:numcp)

		tname <- c("<leaf>", colnames(X))
		splits <-
			matrix(c(rpfit$isplit[, 2:3], rpfit$dsplit),
						 ncol = 5L,
		...
		if (nsplit == 0L) {
			...
			svar <-
				ifelse(index == 0L, 0L, rpfit$isplit[temp, 1L]) # var number
			frame <- data.frame(
		...
		if (method.int == 3L) {
			...
			nodeprob <-
				rpfit$dnode[, numclass + 5L] / sum(wt) # see ginidev.c
			temp <- pmax(1L, init$counts)   # overall class freq in data
			temp <-
				rpfit$dnode[, 4L + (1L:numclass)] %*% diag(init$parms$prior / temp)
			yprob <-
				temp / rowSums(temp)    # necessary with altered priors
			yval2 <-
				matrix(rpfit$dnode[, 4L + (0L:numclass)], ncol = numclass + 1L)
		...
		functions <-
			...
			list(summary = init$summary, print = init$print)
		if (!is.null(init$text))

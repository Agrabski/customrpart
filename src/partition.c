...
int partition(int nodenum, pNode splitnode, double *sumrisk, int n1, int n2)
{
	...
	double twt;
	int nleft, nright;
	int number_of_observations = n2 - n1;
	...
	if (nodenum > 1)
	{
		...
		for (i = n1; i < n2; i++)
		{
			j = rp.sort_index_matrix[0][i]; /* any variable would do, use first */
			if (j < 0)
		...
		(*rp_eval)(number_of_observations, rp.ytemp, me->response_est, &(me->risk), rp.wtemp);
		me->num_obs = number_of_observations;
		me->sum_wt = twt;
		tempcp = me->risk;
	...
	create_primary_split_list(me, n1, n2);
	if (me->primary == NULL)
	{
		/*
	...
	{
		// select a split, then push it to the front
	...
	//print_tree(me, 2);
#endif
	if (rp.maximum_surogate_splits > 0)
		surrogate(me, n1, n2);
	else
		me->surrogate = (pSplit)NULL;
